# @brief Mapping reads to a set of reference genomes, produce tsv mapping results as well
#	as coverage for individual references [on going...]
#
# Prerequisite: 
# 1) gcc-4.7.2 or above
...
#use Digest::MD5 qw(md5);
	...
	GetOptions(
		...
		### Alignment 
		"aln_ref=s"					=> \$option{aln_ref},

		# aligner setting
		"aln_tool=s"				=> \$option{aln_tool},
	...
	unless ($option{aln_ref}) { print "\n\tERR: -aln_ref should be specified\n"; printHelp(); }
	unless ($aln_tool eq "bwa" || $aln_tool eq "mosaik") { print "\n\tERR: -aln_tool should be bwa or mosaik;\n"; printHelp(); }
	
	my ($sec,$min,$hour,$mday,$mon,$year,$wday,$yday,$isdst) = localtime(time);
	unless ($option{quiet}) { print "\n\nTime (min, hr, day) = ($min, $hour, $mday)\n\n"; }

	# --------------------- copy the input fastq files into target dir so that the input would stay intact ----------------------------
	if ($ipfqs) {	copyfiles (\$ipfqs, "$odir/$oprf.pair"); }
	if ($isfqs) { copyfiles (\$isfqs, "$odir/$oprf.single"); }
		
	# --------------------- Process BAM files: convert each BAM file to paired fastq files, and record in $ipfqs -----------------------
	...
	# ----------------------------------------------------------------------------------------------------------------------------------------
	# - preprocessing via mvicuna: duplicate & low complexity read removal & trimming 
	# ----------------------------------------------------------------------------------------------------------------------------------------
	
	if ($option{preproc}) { # enable preprocessing

		unless ($option{quiet}) { print "\n[[[ M-Vicuna: -tasks DupRm,Trim,PairedReadMerge]]]\n\n"; }
		
		# ---------------- run mvicuna for preprocessing ------------------		
		my $drm_op = "$odir./$oprf.drm.1.fq,$opath.drm.2.fq";
		my $trm_op = "$odir./$oprf.trm.1.fq,$opath.trm.2.fq";
		my $trm_os = "$odir./$oprf.trm.s.fq";
		my $mvicuna_op = "$odir/$oprf.drm.trm.1.fq,$odir/$oprf.drm.trm.2.fq";
		my $mvicuna_os = "$odir/$oprf.drm.trm.s.fq";
		...
		my $drm_option = "-drm_perc_sim $option{drm_perc_sim}";
		my $trim_option = "-trm_min_match $option{trm_min_match} -trm_min_rlen $option{trm_min_rlen} -trm_q $option{trm_min_q}";
		...
		system ("$mvicuna_c -batch $option{batch} -pthreads $option{p} -ipfq $ipfqs -opfq $mvicuna_op -osfq $mvicuna_os -tasks \\
			 DupRm,Trim $drm_option -drm_op $drm_op $lc_option -trm_op $trm_op -trm_os $trm_os $trim_option") == 0 or die "mvicuna failed"; 
  ...
  # ----------------------------------------------------------------------------------------------------------------------------------------
	...
	# ----------------------------------------------------------------------------------------------------------------------------------------
	unless ($option{quiet}) { print "\n[[[ ALIGNMENT ]]]\n\n"; }
	my @aln_refs = split (",", $option{aln_ref});
...
#	fix_read_pairs ("$perl_pl $rmhits_pl", $mvicuna_op, "", $quiet_option);
	...
	sub printHelp {
		...
		print "-preproc: enable proprocessing: duplicate and low complexity\n\t\t removal and trimming\n";
		...
		print "\n*** De-contamination using Mosaik, i.e remove ribosomal and UniVec ***\n";
		print "-dc: enable decontamination: remove ribosome and UniVec\n";
		print "-dc_ref_fl: comma separated ref sequence file list\n";
		print "-dc_mosaik_opt: default '-m unique -act 20'; mosaik options\n";
		
		print "\n*** Aligner setting ***\n";
		print "-aln_ref: comma separated reference filelist to align reads against\n";
		print "-aln_tool: default bwa; choices: bwa/mosaik\n";
	...
	############################################################################	
	# copy input fq filelist (comma separated string $ref_in) into the output, 
	# where the output prefix is specified. $ref_in will store the output filelist 
	# as comma separated format 
	sub copyfiles {
		my $ref_in = shift;
		my $oPrefix = shift;
		my $out;
		my @inputlist = split (',', $$ref_in); 
		my $cnt = 0; 
		foreach my $file (@inputlist) {
			my $targetfile = $oPrefix.".".$cnt.".fq";
			if (-e $file) { system ("cp $file $targetfile"); }
			if ($out) { $out .= ",$targetfile"; }
			else { $out = $targetfile; }
			++ $cnt;
		}	
		$$ref_in = $out;
