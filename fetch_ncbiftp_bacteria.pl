...
# -- prepare merged output file $oAll --- 
...
if ($option{draft}) { 
	...
	my $batch_sz = 100;
	my $num_batch = int (scalar(@tgz_files)/$batch_sz); 
		
	my $start;
	my $end;
	my $otmpExtractDIR = $otmpDIR."/extract";
	my $otmpMerged = $otmpDIR."/merged.fa";
	for (my $i = 0; $i < $num_batch; ++ $i) {

		if (-d $otmpExtractDIR) { system ("rm -rf $otmpExtractDIR"); }
		system ("mkdir $otmpExtractDIR") == 0 or die "creating $otmpExtractDIR failed\n";
		system ("rm $otmpMerged");
				
		# --- parallel extraction ---
		
		my $pm = new Parallel::ForkManager($p);
		...
		for (my $j = $start; $j < $end; ++ $j) {
			...
			#print "tar -xzf $file -C $otmpExtractDIR\n"; #tmp added here

			system ("tar -xzf $file -C $otmpExtractDIR");
			$pm->finish;
		}
		$pm->wait_all_children;	

		# --- create tmp extract directory, tmp merged file and carry out merging ---

		system ("find $otmpExtractDIR -name \"*.$type\" -exec cat {} \+ > $otmpMerged");
		system ("cat $otmpMerged >> $oAll");
		
		print "\t $end files complete\n";
	}
	...
	my $pm = new Parallel::ForkManager($p);
	for (my $j = $end; $j < scalar(@tgz_files); ++ $j) {
		my $pid = $pm->start and next;
		my $file = $tgz_files[$j]; 
...
#	print "\nfind $otmpExtractDIR -name \"*.$type\" -exec cat {} \+ > $otmpMerged\n";
	system ("find $otmpExtractDIR -name \"*.$type\" -exec cat {} \+ > $otmpMerged");
#	print "\ncat $otmpMerged >> $oAll\n";
	system ("cat $otmpMerged >> $oAll");
				
	print "$oAll has been completed generated\n";
...
print "\nNum of Genomic, Plasmids: $num_genomes, $num_plasmids (total: $num_total)\n";		
close ($ofh_gen);
close ($ofh_plas);
close ($fh_all);
