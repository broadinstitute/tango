...
public:
	...
	ProcDB (int k, int w, int mcov, int blksz, int blkdist, double perc_span,
			int perc_rep, int batch, bool silent):
		k_(k), w_(w), mcov_(mcov), min_blksz_(blksz), max_blkdist_(blkdist),
		min_perc_span_(perc_span), max_perc_rep_(perc_rep), batch_(batch),
		silent_ (silent) {};
	...
	void detect_spurious_taxa(const std::string& odir,
	...
	void set_kmerinfo(const std::vector<u64ibtuple_t>& rhs) { kmerInfo_ = rhs; }
...
private:
	...
	int w_;              // window size
	int mcov_;           // min freq of kmer pairs
	int min_blksz_;
	int max_blkdist_;
	...
	// ------------- functions related to detect_spurious_taxa() --------
	void get_gi_info_(std::vector<ivtuple_t>& gi_info, const ivec_t& GIs,
			const uu64vec_t& skeletons);
	void validate_spurious_taxa_(ivec_t& indices_rep, ivec_t& indices_span,
			const std::vector<ivtuple_t>& taxa_info,
			const std::string& odir,	const std::string& oprefix);
	void output_spurious_taxa_info_(std::ofstream& ofh,
