...
int main (int argc, char** argv) {
	...
	myPara.procinput(argc, argv);
 	...
 	int batch = 1000000;
 	ProcDB myDB (myPara.get_k(), myPara.get_w(), myPara.get_mcov(),
 			myPara.get_min_blksz(), myPara.get_max_blkdist(),
 			myPara.get_min_perc_span(), myPara.get_max_perc_rep(),
 			batch, myPara.quiet());


 	// test proc_contig_context
 	/*
 	std::vector<u64ibtuple_t> rhs {std::make_tuple (1, 0, false), std::make_tuple (2, 0, true) };
 	u64vec_t skeleton {3, 3, 3, 3, 1, 1, 3, 3, 2 , 3, 3, 3, 3, 1, 3, 1};
	myDB.set_kmerinfo (rhs) ;
	uu64vec_t blocks;
	ivec_t blkspans;
	myDB.identify_valid_skeleton_blocks_ (blocks, blkspans, skeleton);
	for (int i = 0; i < blocks.size(); ++ i) {
		std::cout << blkspans[i] << ": ";
		for (auto& x: blocks[i]) std::cout << x << "\t";
		std::cout << "\n";
	}
	exit (1);
	*/
	...
	myDB.set_kmerinfo (rhs);
 	...
 	myDB.generate_locality_kpairs_per_taxa(taxaKPairs);

  	std::vector<uu64pair_t> rKPairs;
 	myDB.generate_locality_kpairs_in_reads(rKPairs,
 			myPara.get_ipfqs(), myPara.get_isfqs());

 	myDB.set_kmer_multiplicity(taxaKPairs, rKPairs);

 	myDB.detect_spurious_taxa(myPara.get_odir(),	myPara.get_oprefix());
