	...
	#pragma omp parallel
	{
		...
		for (int i = 0; i < num_taxa; ++ i) {
			...
			for (auto& s: skeletons) {
				uu64vec_t blocks;
				ivec_t blkspans;
				identify_valid_skeleton_blocks_(blocks, blkspans, s);
				{ // debug
					for (auto& x: blocks) {
						debug_local_block_sz += x.size();
					}
				}
				proc_blocks_(local_ktaxa, local_kpairtaxa, blocks, i);
			}
		}
		#pragma omp critical
		{
 ...
 *  			a) is separated by adjacent blocks by distance >= max_blkdist_
 *  			b) the distance between adjacent elements in the block < max_blkdist_
 *  			c) the num of elements in the block is >= min_blksz_
 *
 *			// process skeleton to remove --- (x -- x) ---- structure
 */
void ProcDB::identify_valid_skeleton_blocks_(uu64vec_t& blocks,
		ivec_t& blkspans, const u64vec_t& skeleton) {
	...
	for (int i = 0; i < skeleton.size(); ++ i) {
		/*{ //debug print
			std::cout << "cur_blksz, pref_gap_sz, suf_gap_sz, blkspan\n";
			std::cout << i << ":" << cur_block_sz << ", " << prefix_gap_sz << ", "
					<< suffix_gap_sz << ", " << blkspan << "\n";
		}*/
		...
		if (std::binary_search(kmerInfo_.begin(), kmerInfo_.end(),
			...
			++ cur_block_sz;
			if (cur_block_sz >= min_blksz_) prefix_gap_sz = 0;
			suffix_gap_sz = 0;
		} else {
			if (cur_block_sz != 0) ++ suffix_gap_sz;
			else ++ prefix_gap_sz;
			if (suffix_gap_sz > max_blkdist_) {
				if (cur_block_sz >= min_blksz_) {
					blocks.push_back(block);
					blkspans.push_back(blkspan - suffix_gap_sz + 1);
				}
		...
		if (block.size() > 0) ++ blkspan;
	} // for (int i = 0
	if (block.size() >= min_blksz_) {
		blocks.push_back(block);
		blkspans.push_back(blkspan - suffix_gap_sz); // do not need to +1
	}
...
void ProcDB::generate_locality_kpairs_in_reads(
	...
	CountKP kpcounter (kmerInfo_.begin(), kmerInfo_.end(), kmerTaxaID_.begin(),
			kmerTaxaID_.end(), it_debug_kfragfreq, k_, mcov_);
	...
	for (auto& x: rkpcnt) {
		if (std::get<2> (x) >= mcov_)
			rKPairs.push_back(uu64pair_t (std::get<0> (x), std::get<1> (x)));
	}
	if (!silent_) {
		std::cout << "\t# after rm pairs with freq < " << mcov_
				<< ": " << rKPairs.size() << "\n";
	}
 ...
 */
void ProcDB::detect_spurious_taxa(const std::string& odir,
		const std::string& oprefix) {

	if (!silent_) std::cout << "\nDetect spurious taxa...\n";

	// read in skeletons and identify total kmers, m kmers, u kmers, and
	// valid block coverage
	int num_taxa = get_num_taxa_();

	// <m, u, valid_blk_len, total>
	std::vector<ivtuple_t> taxa_info(num_taxa, ivtuple_t{0, 0, 0, 0});
	...
	for (int i = 0; i < num_taxa; ++ i) {
		...
		ivec_t GIs;
		read_skeletons_(GIs, skeletons, skeletonFileList_[i]);
		std::vector<ivtuple_t> gi_info; //<m, u, valid_blk_len, total>
		get_gi_info_(gi_info, GIs, skeletons);

		// get information per taxa
		for (auto& x: gi_info) {
			std::get<0>(taxa_info[i]) += std::get<0>(x);
			std::get<1>(taxa_info[i]) += std::get<1>(x);
			std::get<2>(taxa_info[i]) += std::get<2>(x);
			std::get<3>(taxa_info[i]) += std::get<3>(x);
		}

		/*{ // debug
			if (std::get<3>(taxa_info[i]) <=0) {
				std::cout << "index = " << i << ", taxaID = " << taxaIDs_[i] << "\n";
				exit(1);
			}
		}*/
	}

	if (!silent_) std::cout << "\n\tvalidate spurious taxa\n";
	ivec_t indices_rep, indices_span;
	validate_spurious_taxa_(indices_rep, indices_span, taxa_info, odir, oprefix);

	// write spurious taxa info out to file
	std::string opath = odir + "/spurious_taxa_";
	opath += oprefix + ".txt";
	std::ofstream ofh;
	xny::openfile(ofh, opath);
	ofh << "#Total entries: " << indices_rep.size() + indices_span.size() << "\n";
	ofh << "#multi-kmer criterion: " << indices_rep.size() << " entries\n";
	output_spurious_taxa_info_(ofh, taxa_info, indices_rep);
	ofh << "\n";
	ofh << "#span criterion: " << indices_span.size() << " entries\n";
	output_spurious_taxa_info_(ofh, taxa_info, indices_span);
	xny::closefile(ofh);

	// remove kmers associated with spurious taxa from [kmerTaxaID_]
	ivec_t spurious_indices = indices_rep;
	spurious_indices.insert(spurious_indices.end(), indices_span.begin(),
			indices_span.end());
	std::sort(spurious_indices.begin(), spurious_indices.end());

	bvec_t is_found (get_kmertaxaid_sz_(), false);
	int idx = 0;
	for (auto& x: kmerTaxaID_) {
		if (std::binary_search(spurious_indices.begin(),
				spurious_indices.end(), x.second)) is_found[idx] = true;
		++ idx;
	}
...
/*
	...
	for (int i = 0; i < knum_taxa.size(); ++ i) {
		...
		if (num_total_kmer) {
			...
			if (num_flagged_kmer * 100 / num_total_kmer >= max_perc_rep_) {
				++ num_spurious;
...
/* @brief Using a) max_perc_rep_ and b) min_perc_span_ criteria to
 * determine spurious taxa and output
 */
void ProcDB::validate_spurious_taxa_(ivec_t& indices_rep,
		ivec_t& indices_span,
		const std::vector<ivtuple_t>& taxa_info,
		const std::string& odir,
		const std::string& oprefix){

	// first identify indices for rep and span criterion respectively
	int num_sampled_taxa = 0;
	int idx = 0;
	for (auto& t: taxa_info) { // <m, u, valid_blklen, total>

		int m = std::get<0>(t),
			u = std::get<1>(t),
			valid_blklen = std::get<2>(t),
			total = std::get<3>(t);
		//if (total <= 0) abording ("validate_output_spurious_taxa SC failed");
		if (m + u > 0) {
			if (100.0 * m / (m + u) >= max_perc_rep_) {
				indices_rep.push_back(idx);
			} else if (total > 0 && 100.0 * valid_blklen/total <= min_perc_span_) {
				indices_span.push_back(idx);
			}
			++ num_sampled_taxa;
		}
	...
	if (!silent_) std::cout << "\tTaxa Info: #sampled, #spurious: "
		<< num_sampled_taxa << ", "
		<< indices_rep.size() + indices_span.size() << "\n";

} //validate_output_spurious_taxa

/* @brief	Output spurious taxa_info
 */
void ProcDB::output_spurious_taxa_info_(std::ofstream& ofh,
		const std::vector<ivtuple_t>& taxa_info, const ivec_t& indices) {

	ofh << "TaxaID(index)\tmulti-kmer\tmulti+uni kmer\tspan\ttotal\tName(s)\n";
	ofh << "-----------------------------------------------------------\n";
	for (auto& x: indices) {
		int m = std::get<0>(taxa_info[x]),
			u = std::get<1>(taxa_info[x]),
			valid_blklen = std::get<2>(taxa_info[x]),
			total = std::get<3>(taxa_info[x]);
		if (x >= taxaIDs_.size()) abording ("output_spurious_taxa_info_ SC failed");
		int taxaID = taxaIDs_[x];
		ofh << taxaID << "(" << x << ")" << "\t" <<  m << "\t" << u + m
				<< "\t" << valid_blklen << "\t" << total << "\t"
				<< get_taxa_names_(taxaID) << "\n";
	}
...
/* @brief	Given GIs and skeletons for each taxa, generate data structure
 * for each GI: gi_info<m, u, valid_blk_len, total>
 */
void ProcDB::get_gi_info_(std::vector<ivtuple_t>& gi_info,
		const ivec_t& GIs, const uu64vec_t& skeletons){

	int num_skeleton = skeletons.size();

	for (int i = 0; i < num_skeleton; ++ i) {

		uu64vec_t blocks;
		ivec_t blkspans;
		identify_valid_skeleton_blocks_(blocks, blkspans, skeletons[i]);
		{
			for (int x = 0; x < blocks.size(); ++ x) {
				if (blocks[x].size() > blkspans[x]) {
					std::cout << "WRONG\n";
					exit (1);
				}
			}
		}
		int sz = skeletons[i].size(),
			span = 0,
			m = 0,
			u = 0;
 		for (int j = 0; j < blocks.size(); ++ j) {
 			span += blkspans[j];
 			int blk_m = 0, blk_u = 0;
			for (auto& kmer: blocks[j]) {
				if (get_kmerinfo_flag_(kmer)) ++ blk_m;
				else ++ blk_u;
			}
			// guilty by association
			if (100.0 * blk_m/(blk_m + blk_u) >= max_perc_rep_) m += blk_m + blk_u;
			else {
				m += blk_m;
				u += blk_u;
			}
 		...
 		if (i == 0) gi_info.push_back(ivtuple_t{m, u, span, sz});
 		else {
 			if (GIs[i] == GIs[i - 1]) {
 				std::get<0>(gi_info.back()) += m;
 				std::get<1>(gi_info.back()) += u;
 				std::get<2>(gi_info.back()) += span;
 				std::get<3>(gi_info.back()) += sz;
 			} else gi_info.push_back(ivtuple_t{m, u, span, sz});
 		}
	}// for
...
std::string ProcDB::get_taxa_names_(int taxaID) {
	...
	if (dist >= taxa_names_.size()) {
		...
		std::cout << "taxa_names_[dist].first, taxaID: "
				<< taxa_names_[dist].first << ", " << taxaID << "\n";
		warning ("ProcDB::detect_spurious_taxa taxaID not documented");
...
void ProcDB::assign_frags(const strvec_t& ipfqs,	const strvec_t& isfqs,
	...
	*/
	...
	for (int i = 0; i < num_taxa; ++ i) {
		...
		if ((total_adjusted_cnt == 0 && taxacnts[i] > 0) ||
			(total_adjusted_cnt > 0 && (taxacnts[i] * 1000 / total_adjusted_cnt > 0))) {
