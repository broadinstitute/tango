...
namespace xny{
	 ...
	 * strands according to the [code]:
	 * code =
	 * 		 0: default, fwd strand if first char is A/C, rvc otherwise
	 * 		 2: kmers are retained in the order of
	 * 		 	fwd0, rv0, fwd1, rv1 ...
	 * 		 3: only fwd strand fw0, fw1 ... fw_i
	 * 		 4: only rv strand rv0 rv1 ...rv_i, i is w.r.t the fwd strand
	 ...
	 */
	template <typename outputIter, typename out_t>
	void get_bitkmer_by_alphabet (const std::string& seq,
			outputIter out, std::deque<bool>& is_valid, int k, int code = 0) {
		...
		case 3:
		case 4:
			is_valid = std::deque<bool> (len, true);
			break;
		case 2:
			is_valid = std::deque<bool> (2 * len, true);
			break;
		default:
			std::cout << "In func get_bitkmer, [code] out of range\n";
			exit(1);
		...
		out_t ID = 0, rcID = 0;
		int j = 0; // location on the string
		bool flag = false; //true: the previous kmer contains no Ns
		while (j < len){
			if (flag){
				int c = char2bits (*(addr + j + k -1));
				char first_c = std::toupper(*(addr + j));
				if (c == -1) {
					...
					case 2:
						for (int l = j; (l < j + k) && (l < len); ++ l){
							is_valid[2*l] = is_valid[2*l + 1] = false;
							*out = 0; ++ out;
							*out = 0; ++ out;
						}
						break;
				...
				if (str2ID (ID, addr + j, k)) {
					...
					case 4:
						...
						*out = 0; ++ out;
						break;
					case 2:
						is_valid[2*j] = is_valid[2*j + 1] = false;
						*out = 0; ++ out;
						*out = 0; ++ out;
						break;
					default:
	 ...
	 ********************************************************************/
	template <typename outputIter>
	void get_bit_gapped_seeds (outputIter out, const std::deque<bool>& seed,
